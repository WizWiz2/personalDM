...
export function CampaignLibraryPage() {
  ...
  const renderCover = (campaign: Campaign, compact = false) => {
    ...
    return <GeneratedPixelArt src={src} alt={`Обложка ${campaign.name}`} fallback={fallback} active />
  }
    ...
    <div className="global-page">
      ...
      <div className="global-content campaign-library">
        ...
        {!loading && !error && campaigns.length === 0 && (
          <EmptyState title="Пока нет кампаний" text="Создайте первую историю — мастер поможет придумать мир и героя." action={<button className="btn primary" onClick={() => setModal(true)}>Создать кампанию</button>} />
        )}
                ...
                {campaigns.map((campaign) => (
                  <article className="campaign-card" key={campaign.id}>
                    ...
                    <div className="campaign-card-body">
                      <span className="eyebrow">{campaign.current_scene_id ? 'Кампания' : 'Подготовка'}</span>
                      <h3>{campaign.name}</h3>
        ...
        <div className="modal-backdrop" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && setModal(false)}>
          <form ref={createDialogRef} className="modal" role="dialog" aria-modal="true" aria-labelledby="create-campaign-title" onSubmit={createCampaign}>
            <div className="modal-head">
              <div><span className="eyebrow">Новая история</span><h2 id="create-campaign-title">Создать кампанию</h2></div>
              <button type="button" className="icon-btn" onClick={() => setModal(false)} aria-label="Закрыть"><Icons.close /></button>
            </div>
            <label>Название<input ref={nameInputRef} value={name} onChange={(e) => setName(e.target.value)} placeholder="Например: Тайна портового города" /></label>
            <label>Описание кампании<textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="О чём эта история" rows={4} /></label>
          ...
          <div className="modal danger-modal" role="dialog" aria-modal="true" aria-labelledby="delete-campaign-title">
            <div className="modal-head">
              <div>
                ...
                <h2 id="delete-campaign-title">Удалить «{deleteTarget.name}»?</h2>
              </div>
            ...
            <div className="modal-actions">
              ...
              >
                {deletingId === deleteTarget.id ? 'Удаляем…' : 'Удалить кампанию'}
              </button>
            </div>
          </div>
        </div>
