...
import type { CharacterCard, Fact, SceneState } from '../api/types'
...
type WorldItem = { id: string; name: string; subtitle: string; description?: string | null }
  ...
  const characterItems = useMemo<WorldItem[]>(() => {
    ...
    return scene.participant_ids.flatMap((id, index) => {
      ...
      return [{
        id,
        name,
        subtitle: relationship?.relation_type || 'Участник текущей сцены',
        description: relationship?.description || 'Сейчас находится в одной сцене с героем.',
      }]
    })
  }, [scene, card, campaign.player_character_id])
    ...
    <div className="page-content world-content">
      ...
      {!loading && !error && <div className="world-layout">
        <div className="world-index">
          ...
          {filtered.map((item) => <button key={item.id} className={`world-index-item ${selectedItem?.id === item.id ? 'active' : ''}`} onClick={() => setSelected(item.id)}>{tab === 'characters' ? <span className="tiny-portrait"><PixelPortrait seed={item.name} /></span> : <span className="tiny-scene"><PixelScene seed={item.name} compact /></span>}<span><strong>{item.name}</strong><small>{item.subtitle}</small></span></button>)}
        </div>
        <article className="world-detail">
          {selectedItem ? <>
            {tab === 'characters' ? <div className="world-portrait"><PixelPortrait seed={selectedItem.name} /></div> : tab === 'locations' ? <div className="world-scene"><PixelScene seed={selectedItem.name} /></div> : null}
